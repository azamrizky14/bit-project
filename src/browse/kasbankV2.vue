  ...
  <main id="content" role="main" class="main">
    <div class="content container-fluid p-3 ps-0 pt-0" ref="contentDiv">
      ...
      <template v-if="isDataLoaded">
        <tableBrowse :header="header" :data="filteredTableData" :fields="tableFields" :settings="hotSettings" :routingLink="routing"
...
import tableBrowse from '../components/tableBrowse.vue'
export default {
  components: {
    tableBrowse
  },
  ...
  async mounted() {
    ...
    this.posts = await get1(this.$route.name.replace('browse-',''));
    const transData = (await get0(this.$route.name.replace('browse-','').replace('-v2',''))).masterData;
  ...
  methods: {
    ...
    async getComponent() {
      this.component = this.posts.transProperties.find(({ dataName }) => dataName === "menuListReport").dataMicro
    },
    async getHeader() {
      this.header = this.posts.transProperties.find(({ dataName }) => dataName === "headerReport").dataMicro
    },
    getFieldTable() {
