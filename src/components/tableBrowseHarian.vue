  ...
  <div>
      ...
      <b-form-group label-for="coaKasbank" invalid-feedback="Condition Harus Diisi!">
        <div class="row">
          <div class="col-8 d-flex align-items-center">
            <multiselect class="my-1" id="coaKasbank" :options="optionsData" placeholder="Pilih " v-model="newData"
              :multiple="false" :show-labels="false" :allow-empty="required" :custom-label="customLabel"
              @input="listData">
            </multiselect>
          </div>
          <div class="col-4 d-flex align-items-center">
            <b-form-input type="date" class="" v-model="getDate" @change="getData"></b-form-input>
          </div>
        </div>
      </b-form-group>
    </div>
    ...
    <div class="card">
      <div class="card-header bg-light p-2">
        <div class="row">
          <div v-if="filterOption === 'Data'" class="col-10 d-flex align-items-center">
            <select v-model="filterFields" style="font-size: 12px; max-width: 125px;"
            ...
            <select v-model="filterSort" class="mx-2 form-select form-select-sm py-1 px-2"
            ...
            <div class="input-group" style="width: 200px;">
              ...
              <input v-model="filter" type="text" class="form-control form-control-sm p-1" placeholder="Cari Data..."
            ...
            <div v-for="list in filterList" class="input-group ms-2" style="width: 125px;">
              <input v-model="list.filter" :disabled="filter === ''" type="text" class="form-control form-control-sm p-1"
          ...
          <div v-else-if="filterOption === 'Period'" class="col-10 d-flex align-items-center">
            ...
            <div class="input-group" style="width: 125px;">
              <input v-model="startDate" type="date" class="form-control form-control-sm p-1" placeholder="Cari Data..."
                aria-label="Recipient's username" aria-describedby="basic-addon2" style="font-size: 12px;">
            </div>
            <span style="font-size: 12px;" class="mx-3">Sampai</span>
            <div class="input-group" style="width: 125px;">
              <input v-model="endDate" type="date" class="form-control form-control-sm p-1" placeholder="Cari Data..."
                aria-label="Recipient's username" aria-describedby="basic-addon2" style="font-size: 12px;">
            </div>
          ...
          <div class="col-2 d-flex justify-content-end" style="z-index: 2">
            ...
            <b-button-group>
              <b-dropdown id="dropdown1" variant="primary" class="dropdown-without-icon" size="xs" right>
                ...
                <b-dropdown-item :active="filterOption === 'Data'" @click="filterOption = 'Data'">Filter by
                  Data</b-dropdown-item>
                <b-dropdown-item :active="filterOption === 'Period'" @click="filterOption = 'Period'">Filter by
                  Period</b-dropdown-item>
              </b-dropdown>
      ...
      <div class="card-body p-2">
        ...
        <hr class="my-2">
        <hot-table :settings="settings" :data="hotData" :rowHeaders="true" :colHeaders="true" :colWidths="tableWidth"
          style="z-index:0"></hot-table>
      </div>
    </div>
  </div>
</template>
...
<script>
import { getE, getG } from './services/components'
      ...
      const data = {
        ...
        transData: this.optionsData.flatMap(item => item.trans).map(x => {
          ...
          if (x.transType === 'debet') {
            ...
            minus.debet = x.transAmount.slice(1)
            minus.kredit = 0
          }
      ...
      if (this.newData == null || this.newData.length === 0) {
        ...
        cok = this.optionsData.flatMap(item => item.trans).map(x => {
          ...
          if (x.transType === 'debet') {
            ...
            minus.transOut = x.transAmount.slice(1)
            minus.transIn = 0
          }
          return minus
        })
