  ...
  <div>
    ...
    <div class="card">
      ...
      <div class="card-body p-2 printData">
        <h2 class="text-center">Laporan Neraca Saldo Bulanan</h2>
        <printNeracaSaldo :title="print" :data="hotData" :saldoAwal="saldoAwal"></printNeracaSaldo>
      </div>
    </div>
  </div>
</template>
...
<script>
import { getE, getG, getInventoryReport, getNeracaSaldo } from './services/components'
...
import printNeracaSaldo from './print/printNeracaSaldo' 

export default {
  components: {
    numberCounter, printNeracaSaldo
  },
  props: {
    header: {
      type: Array,
...
tr {
  font-size: 10px
    ...
    async getData() {
      const tes = await getInventoryReport(this.getDate)
      this.hotData = await getNeracaSaldo(this.getDate)
      console.log('inv data', this.hotData)
      console.log('saldo data', tes)
