  ...
  <div>
    ...
    <div class="card">
      ...
      <div class="card-body p-2 printData">
        <h2 class="text-center">Laporan Inventory Stock Detail</h2>
        <printStockInventory :title="print" :data="hotData" :saldoAwal="saldoAwal"></printStockInventory>
      </div>
    </div>
  </div>
</template>
...
<script>
import { getE, getG, getInventoryStockReport } from './services/components'
...
import printStockInventory from './print/printStockInventory' 

export default {
  components: {
    numberCounter, printStockInventory
  },
  props: {
    header: {
      type: Array,
...
tr {
  font-size: 10px
    ...
    async getData() {
      this.hotData = await getInventoryStockReport(this.getDate)
