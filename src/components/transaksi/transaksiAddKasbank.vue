...
import { getF, getB, getD, addKasbank } from '../services/components';
...
export default {
  ...
  async mounted() {
    ...
    this.optionsKasbank = this.dataList.saldoAwal
  ...
  methods: {
    ...
    findVoucher() {
      if (this.kasbank.coaCode && this.transType !== '') {
        ...
        } else if (this.transType === 'pengeluaran' && this.kasbank.coaCode.includes('1.1.1.1.')) {
          this.transNo = this.voucher[0].cashOut
        }
        else if (this.transType === 'pemasukan' && this.kasbank.coaCode.includes('1.1.1.2.')) {
          this.transNo = this.voucher[0].bankIn
        } 
        else if (this.transType === 'pengeluaran' && this.kasbank.coaCode.includes('1.1.1.2.')) {
    ...
    sendData(event) {
      ...
      if (finalHeader.type === 'pengeluaran') {
        finalHeader.type = 'kredit'
      } else if (finalHeader.type === 'pemasukan') {
