...
registerAllModules();
import { addPO } from '../services/components'
  ...
  methods: {
    ...
    sendData(event) {
      ...
      const finalTable = this.excelData
        ...
        .map(item => ({
          PrId: item.PrCode._id,
          PrNo: item.PrCode.PrNo,
          dataPO: [finalHeader].map(x => {
            const data = {
              PoDomain: '648da0edc3889697b16e8675', // nanti dibuat otomatis
              PoNo: "",
              PoReff: "",
              PoUserCreated: localStorage.getItem('_id'),
              PoSupplier: x.supplier._id,
              PoCreditTerms: x.creditTerms,
              PoShipto: x.shipTo._id,
              poDate: x.datePO,
              PoNeedDate: x.dateNeedPO,
              PoNotes: finalNote.note,
              PoRouting: this.poRouting[0].routingConfirm,
              PoItemDetails: [item.itemCode].map(x => ({ ...x, itemPrice: item.harga.toString() }))
            }
      ...
      if (doneValidator) {
        this.$swal({
          ...
          if (result.isConfirmed) {
            this.alertLoading();
            try {
              const voucher = await addPO(finalTable);
              this.successAdd();
              this.refreshFields();
