  ...
  <form @submit="sendData">
    <div class="row my-1">
      ...
      <div class="col-4 text-center">
        <span class="h2">Buat {{ posts.transTitle }}</span>
      </div>
            ...
            <div v-for="micro in field.componentMicro" :class="['', micro.microClass]">
              ...
              <template v-else-if="micro.microType === 'multiselect'">
                <b-form-group :label="micro.microName + ' :'" :label-for="micro.microName" label-cols-sm="4"
                  ...
                  <template v-else-if="micro.key === 'requester'">
                    <multiselect class="my-1" :id="micro.microName" v-model="micro.value" :options="micro.microOptions"
                      @input="selectCustomer(micro.value)" placeholder="Select one" :multiple="false" :show-labels="false"
                      required :custom-label="customLabel">
                    </multiselect>
                  </template>
                  <template v-else>
                    <multiselect class="my-1" :id="micro.microName" v-model="micro.value" :options="micro.microOptions"
    ...
    <div class="row mb-3">
      ...
      <div class="col-md-6 row p-0 scale-9" style="transform-origin: right top">
        <div v-for="field in component" :class="[field.componentClass, 'd-none']"
...
registerAllModules();
import { addPR } from '../services/components'
...
export default {
  ...
  async mounted() {
    this.prRouting = await get4('Purchase Request')
  },
  methods: {
    refreshFields() {
    ...
    sendData(event) {
      ...
      const finalTable = this.excelData
        ...
        .map(item => ({
          ...
          // itemQtyReceipt: item.itemQty.toString(),
          itemId: item.itemId,
          itemNoted: '-'
        }))
      // final Note
      ...
      // final Data
      const finalData = {
        PrDomain: "648da0edc3889697b16e8675", // nanti dibuat otomatis
        PrNo: "0",
        PrReff: '',
        PrUserCreated: localStorage.getItem('_id'),
        PrRequestBy: finalHeader.requester.userName,
        PrRequestByDept: finalHeader.PrRequestByDept,
        PrPurpose: finalHeader.PrPurpose,
        PrNeedDate: finalHeader.PrNeedDate,
        PrNotes: finalNote.note,
        PrDate: finalHeader.PrDate,
        PrItemDetails: finalTable,
        PrRouting: this.prRouting[0].routingConfirm
      }
      ...
      if (doneValidator) {
        this.$swal({
          ...
          if (result.isConfirmed) {
            this.alertLoading();
            try {
            const voucher = await addPR(finalData);
              this.successAdd();
              this.refreshFields();
