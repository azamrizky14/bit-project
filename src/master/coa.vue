        ...
        >
          ...
          <div class="row" v-else>
            <div class="row col-lg-12">
              ...
              <div class="col-7 ps-1">{{ infoModal.content.coaType }}</div>
              <div class="col-5 d-flex justify-content-between">
                <span>Kode COA</span>:
              </div>
              <div class="col-7 ps-1">{{ infoModal.content.coaCode }}</div>
              <div class="col-5 d-flex justify-content-between">
                <span>Nama COA</span>:
              </div>
              <div class="col-7 ps-1">{{ infoModal.content.coaName }}</div>
            </div>
            <template>
              <table class="table table-bordered">
                ...
                <tbody>
                  <template v-if="infoModal.content.coaFaAmount">                    
                  <tr v-for="data in infoModal.content.coaFaAmount">
                    <td>{{ data.FaPriceYear }}</td>
                    <td>{{ data.FaPriceMonth }}</td>
                    <td>{{ numberFormat(data.FaPriceBegining) }}</td>
                  </tr>
                  </template>
                </tbody>
              </table>
        ...
        >
          <form ref="form" class="row" @submit.stop.prevent="handleEditSubmit">
            ...
            <div
              class="col-4 d-flex align-items-center justify-content-between"
            >
              <h4 class="m-0">Nama COA</h4>
              <h3>:</h3>
            </div>
            <div class="col-md-8 my-1">
              <b-form-group
                label-for="nameCOA"
                invalid-feedback="Nama COA Harus Diisi!"
                :state="descState"
              >
                <b-form-input
          ...
          <form ref="form" class="row p-5" @submit.stop.prevent="handleSubmit">
            ...
            <div class="col-md-5 mb-2">
              <b-form-group
                ...
                invalid-feedback="Saldo Awal Harus Diisi!"
              >
                <b-form-input
                  type="month"
                  id="ItemAmount"
            ...
            <div class="col-md-7 mb-2">
              <b-form-group
                ...
                label-for="ItemAmount"
                invalid-feedback="Saldo Awal Harus Diisi!"
                :state="provState" v-format-number
              >
                <b-input-group prepend="Rp.">
                <b-form-input
                  placeholder="100.000"
...
export default {
  data() {
    return {
      ...
      fields: ["coaCode", "coaName", "coaType", "opsi"],
      sorts: [],
      selected: ["coaCode", "coaName", "coaType", "opsi"],
      Item: [],
      process: [],
      microservice: [],
      filter: "",
  ...
  methods: {
    ...
    async ItemTable() {
      ...
      this.Item = data.dataList.filter((x) => x.coaDeleted === "no");
      ...
      this.$swal({
        ...
        if (result.isConfirmed) {
          ...
          if (typeData === "Item") {
            ...
            const data = {
              coaCode: this.type.kode+'.'+this.desc,
              coaName: this.name,
              coaType: this.type.name,
              coaDomain: '648da0edc3889697b16e8675',
              coaFaAmount: [
                {
                  FaPriceYear: year,
                  FaPriceMonth: month,
                  FaPriceBegining: this.prov.replace(/,/g, ''),
                  FaPriceIn: '0',
                  FaPriceOut: '0',
                  FaPriceEnd: '0'
                }
    ...
    edit(Item, index, button) {
      this.editModal.title = `Edit Data: ${Item.coaCode} (${Item.coaCode})`;
      this.editModal.content = Item;
      this.$root.$emit("bv::show::modal", this.editModal.id, button);

      this.id = this.editModal.content._id;
      this.name = this.editModal.content.coaCode;
      this.desc = this.editModal.content.coaName;
      this.type = { name: this.editModal.content.coaType };
      this.currentDate = this.editModal.content.coaFaAmount[0].FaPriceYear + '-' + this.editModal.content.coaFaAmount[0].FaPriceBegining;
      this.prov = this.formatNumber(this.editModal.content.coaFaAmount[0].FaPriceBegining)
    },
    ...
    handleSubmitProc() {
      if (this.name && this.type && this.desc && this.prov) {
    ...
    async info(Item, index, button) {
      this.infoModal.title = `Detail Data: ${Item.coaName} (${Item.coaCode})`;
