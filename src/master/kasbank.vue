        ...
        >
          ...
          <div class="row" v-else>
            <div class="row col-lg-12">
              ...
              <div class="col-7 ps-1">{{ infoModal.content.kasbankCode }}</div>
              <div class="col-5 d-flex justify-content-between">
                <span>Tipe COA</span>:
              </div>
              <div class="col-7 ps-1">{{ infoModal.content.kasbankAmount }}</div>
              <div class="col-5 d-flex justify-content-between">
                <span>Nama COA</span>:
              </div>
              <div class="col-7 ps-1">{{ infoModal.content.kasbankName }}</div>
            </div>
          </div>
        ...
        >
          <form ref="form" class="row" @submit.stop.prevent="handleEditSubmit">
            ...
            <div class="col-md-8 my-1">
              <b-form-group
                label-for="typeCOA"
                invalid-feedback="Tipe COA Harus Diisi!"
                :state="typeState"
              >
                <multiselect
                  id="typeCOA"
            ...
            <div class="col-md-8 my-1">
              <b-form-group
                label-for="codeCOA"
                invalid-feedback="Kode COA Harus Diisi!"
                :state="NameState"
              >
                <b-form-input
                  id="codeCOA"
          ...
          <form ref="form" class="row" @submit.stop.prevent="handleSubmit">
            <div class="col-md-12 mb-2">
              <b-form-group
                label="Jenis Kasbank"
                label-for="ItemType"
                invalid-feedback="Jenis Kasbank Harus Diisi!"
                :state="typeState"
              >
                <multiselect
                  id="itemType"
            ...
            <div class="col-md-12 mb-2">
              <b-form-group
                label="Nama Kasbank"
                label-for="ItemName"
                invalid-feedback="Nama Kasbank Harus Diisi!"
                :state="descState"
              >
                <b-form-input
                  id="ItemName"
            ...
            <div class="col-md-12 mb-2">
              <b-form-group
                ...
                label-for="codeCOA"
                invalid-feedback="Tanggal Dibuat Harus Diisi!"
                :state="addressState"
              >
                <b-form-input
                  type="date"
...
export default {
  data() {
    return {
      ...
      fields: ["kasbankCode", "kasbankJenis", "kasbankName", "kasbankCreatedDate", "kasbankAmount", "kasbankStatus", "opsi"],
      sorts: [],
      selected: ["kasbankCode", "kasbankJenis", "kasbankName", "kasbankCreatedDate", "kasbankAmount", "kasbankStatus", "opsi"],
      Item: [],
      process: [],
      microservice: [],
      filter: "",
  ...
  methods: {
    ...
    async ItemTable() {
    const data = await get0(this.$route.name.replace('master-',''));
      this.Item = data.masterData.filter((x) => x.kasbankDeleted === "no");
      this.optionsCategory = ['Cash', 'Bank'];
    ...
    alertAdd(typeData) {
      this.$swal({
        ...
        if (result.isConfirmed) {
          ...
          if (typeData === "Item") {
            const data = {
              kasbankCode: this.name,
              kasbankName: this.desc,
              kasbankAmount: this.type.name,
            };
            try {
              console.log(data);
              await addCOA(data);
    ...
    edit(Item, index, button) {
      this.editModal.title = `Edit Data: ${Item.kasbankCode}`;
      this.editModal.content = Item;
      this.$root.$emit("bv::show::modal", this.editModal.id, button);

      this.id = this.editModal.content._id;
      this.name = this.editModal.content.kasbankCode;
      this.desc = this.editModal.content.kasbankName;
      this.type = { name: this.editModal.content.kasbankAmount };
    },
    ...
    proc(Item, index, button) {
      this.procModal.title = `Proses Dari: ${Item.kasbankCode}`;
    ...
    async info(Item, index, button) {
      this.infoModal.title = `Detail Data: ${Item.kasbankCode}`;
  ...
  computed: {
    activeFields() {
      ...
      return this.sorts.map((v) => {
        ...
        if (v === "kasbankCode") {
          Item["label"] = "Kode Kasbank";
        }
        if (v === "kasbankJenis") {
          Item["label"] = "Jenis";
        }
        if (v === "kasbankName") {
          Item["label"] = "Nama Kasbank";
        }
        if (v === "kasbankCreatedDate") {
          Item["label"] = "Tgl. Dibuat";
        }
        if (v === "kasbankAmount") {
          Item["label"] = "Saldo";
        }
