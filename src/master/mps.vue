      ...
      <div class="content container-fluid">
        <!-- Page Header -->
            ...
            <!-- Header -->
                    ...
                    <div class="row" v-else>
                        <div class="row col-lg-4">
                            <label class="avatar avatar-xxl overflow-hidden d-flex justify-content-center" for="editAvatarUploaderModal">
                                <img :src="infoModal.content.icon" alt="icon">
                            </label>
                        </div>
                        <div class="row col-lg-8">
                        <div class="col-5 d-flex justify-content-between"><span>ID</span>:</div>
                        <div class="col-7 ps-1">{{ infoModal.content._id }}</div>
                        <div class="col-5 d-flex justify-content-between"><span>Nama jadwal produksi</span>:</div>
                ...
                <b-modal :id="editModal.id" :title="editModal.title" @hidden="resetEditModal"
                    ...
                    <form ref="form" class="row" @submit.stop.prevent="handleEditSubmit">
                        ...
                        <div class="col-md-8 my-1">
                            <b-form-group
                                label-for="ItemName"
                                invalid-feedback="Nama jadwal produksi Harus Diisi!"
                                :state="NameState"
                                >
                                <b-form-input
                                    id="ItemName"
                ...
                <b-modal :id="editProc.id" :title="editProc.title" @hidden="resetEditProc" ok-only no-close-on-esc no-close-on-backdrop hide-header-close>                    
                    <form ref="form" @submit.stop.prevent="handleMicroEdit" v-if="dataId !== '' && procId !== '' && microId !==''">
                ...
                <template #modal-footer  v-if="dataId !== '' && procId !== '' && microId !==''">
                    <b-button variant="secondary" @click="$bvModal.hide('edit-modal-proc')">Kembali</b-button>
                ...
                <b-modal id="modal-add" size="lg" title="Tambah jadwal produksi" 
                    no-close-on-esc
                    ...
                    <form ref="form" class="row" @submit.stop.prevent="handleSubmit">
                        ...
                        <div class="col-md-6 mb-2">
                                <b-form-group
                                label="Jenis Item (Tidak Bisa Dirubah)"
                                label-for="ItemType"
...
import axios from "axios";
  import { getOptJenis, getUsaha, addUsaha, addProc, addMicro, updateUsaha, updateProc, updateMicro } from "./services/usaha";
    export default {
        data(){
    return {
        microData: true,
...
alertDelete(typeData, data) {
  ...
  computed: {
    activeFields(){
     ...
     return this.sorts.map(v => {
        ...
        if(v === 'domainName') {
            Item['label']="Nama jadwal produksi"
        }
        if(v === 'domainDesc') {
  ...
  async mounted() {
    ...
    this.$store.commit('setTopbarTitle', "Data jadwal produksi");
