<template>
  <main id="content" role="main" class="main">
    <div class="content container-fluid p-3 ps-0 py-0" ref="contentDiv">
      <transaksiAddJE :posts="posts" :coa="this.coa" :component="refreshComponent" :hotSettings="hotSettings" :widthTable="contentDivWidth" :excelData="excelData" :refreshMethods="onRefresh"></transaksiAddJE>
    </div>
  </main>
</template>
...
import transaksiAddJE from '../../components/transaksi/transaksiAddJE.vue'
...
export default {
  ...
  async mounted() {
    ...
    this.hotSettings.columns.find(x => x.data === 'Code').source = this.coa.coaNonBank.map(x => x.coaCode)
    this.hotSettings.columns.find(x => x.data === 'Name').source = this.coa.coaNonBank.map(x => x.coaName)
