<template>
  <main id="content" role="main" class="main">
    <div class="content container-fluid p-3 ps-0 py-0" ref="contentDiv">
      <transaksiDetailKasbank :posts="posts" :component="component" :hotSettings="hotSettings" :widthTable="contentDivWidth" :dataDetail="dummyData" :refreshMethods="refreshValue"></transaksiDetailKasbank>
    </div>
  </main>
</template>
...
import transaksiDetailKasbank from '../../components/transaksi/transaksiDetailKasbank.vue'
...
export default {
  ...
  components: {
    transaksiDetailKasbank, HotTable
  },
  data() {
    return {
      dummyData: [],
  ...
  methods: {
    ...
    sumProperty(property) {
      return this.dataDetail.reduce((sum, entry) => parseInt(sum) + entry[property], 0);
    },
    getTotal(property) {
    ...
    findData() {
      this.detailData = Object.entries(this.dataDetail[0]).map(([key, value]) => ({
        key,
        value 
      }));
      console.log('coba',this.detailData)
