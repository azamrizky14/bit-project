        ...
        <div class="content container-fluid p-3 ps-0" ref="contentDiv">
            ...
            <div class="row mb-3 px-3">
              ...
              <div class="col-md-6 row p-0 scale-9" style="transform-origin: right top">
                <div v-for="field in refreshComponent" :class="[field.componentClass]" v-if="field.componentName === 'fieldTotal'">
...
export default {
  ...
  async mounted() {
    ...
    this.$store.commit('setTopbarTitle', 'Edit - ' + this.posts.transTitle);
    ...
    const startIndex = this.dataDetail.dataTable.length;
    const endIndex = 1000;
    for (let i = startIndex; i<endIndex; i++) {
      const key = `row_${i}`;
      const objectAdd = {...dataObject, key}
      this.dataDetail.dataTable.push(objectAdd)
    }
  ...
  methods: {
    ...
    sumProperty(property) {
      return this.dataDetail.dataTable.reduce((sum, entry) => parseInt(sum) + entry[property], 0);
    },
    getTotal(property) {
    ...
    fetchPosts() {
      ...
      var mockResponse = [
        {
            transTitle: 'Production Planning',
            transRouting: 'production-planning',
          transProperties: [
            {dataName: 'menuAdd', dataMicro: [
              ...
              {componentName: 'routingButton', componentTitle: 'button', componentClass: 'col-md-12', componentMicro: [
                ...
                {key: 'save',microName: 'Edit', microRouting: 'edit', microText: 'Edit', microClass: 'btn-success'}
              ]},
            ]}
          ],
          transData: [
