      ...
      <template v-if="isDataLoaded">
        <div class="card">
          ...
          <div class="card-body p-2">
            ...
            <b-table striped hover :items="dataListJE" :fields="tableHeader" label-sort-asc="" label-sort-desc=""
              class="myTable listTable">
              <template #cell(PoSupplier)="data">
                {{ data.item.PoSupplier.userName }}
              </template>
              <template #cell(transEffDate)="data">
                {{ formatIndDate(data.item.transHysItem[0].itemHysEffDate) }}
              </template>
              <template #cell(coaType)="data">
                <template v-if="data.item.coaTrans[0].transType">
                  Kredit
                </template>
                <template v-else>
                  Debet
              ...
              <template #cell(transHysItem)="data">
                <div class="inner-table">
                  <table class="table table-bordered">
                    <thead>
                      <tr>
                        <th style="width: 125px">Nama Item</th>
                        <th>Receipt To</th>
                        <th style="width: 75px;">Qty. Order</th>
                        <th style="width: 75px;">Qty. Recive</th>
                        <!-- Tambahkan kolom lain di sini sesuai kebutuhan -->
                      </tr>
                    </thead>
                    <tbody>
                      <tr v-for="(item, index) in data.item.transHysItem" :key="index">
                        <td>{{ item.itemHysName }}</td>
                        <td>
                          <template v-if="item.itemHysQtyReceiptTo">
                            <template v-if="item.itemHysQtyReceiptTo.length > 0">
                            {{ item.itemHysQtyReceiptTo[0].itemLocation.name }}
                          </template>
                          </template>
                        </td>
                        <td>{{ item.itemHysQtyOrder }}</td>
...
import { get1, getPOReceipt } from '../components/services/components'
    ...
    (this.tableHeader = [
      {
        ...
        label: "No. PO Receipt",
        sortable: true,
        class: "align-top", thClass: "text-center", thStyle: "width:125px"
      },
      {
        key: "transHysReff",
        label: "Kode PO",
      ...
      },
      {
        ...
        label: "List PO",
        sortable: true,
        class: "align-top", thClass: "text-center", thStyle: ""
      },
      {
        key: "transHysUser.userName",
        label: "Receipt By",
        sortable: true,
        class: "align-top", thClass: "text-center", thStyle: ""
      },
      {
        ...
        label: "Receipt Date",
        sortable: true,
        class: "align-top", thClass: "text-center", thStyle: "width:100px"
      },
      { key: "actions", label: "Actions", class: "align-top", thClass: "text-center", thStyle: "width:75px"},
    ]),
      // this.listUser = await getUser()
      // this.listSupplier = await getSupplier()
      this.dataListJE = await getPOReceipt();
      // console.log('coba',this.dataListJE)
