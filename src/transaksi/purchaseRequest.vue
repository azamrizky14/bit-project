...
<script>
import { get1, getI } from '../components/services/components'
...
export default {
  ...
  async mounted() {
    ...
    this.PrList = await getI()
    console.log(this.PrList)
    this.getComponent();
    const trueFields = this.getFieldTable();
    this.hotSettings.columns = trueFields;
    this.hotSettings.columns.find(x => x.data === 'PrNeedDate').dateFormat = 'DD/MM/YYYY'
    console.log(this.hotSettings.columns.find(x => x.data === 'PrNeedDate'))
    this.isDataLoaded = true;
    this.routing = this.posts.transRouting
    this.data = this.PrList
    this.$nextTick(() => {
  ...
  methods: {
    ...
    getFieldTable() {
      ...
      const trueFields = fields.map((x) => {
        if (x.microType === 'date') {
          return { data: x.key, title: x.microName, type: x.microType, dateFormat: x.microFormat, width: 110 }
