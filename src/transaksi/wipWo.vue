...
export default {
  ...
  async mounted() {
    const tes = await getAllWObyDomain();
  ...
  methods: {
    ...
    buttonRenderer(instance, td, row, col, prop, value, cellProperties) {
      ...
      button1.className =
        "btn btn-xs btn-outline-secondary px-1 py-0 rounded-circle";
      ...
      button2.innerHTML = '<i class="bi-box-seam-fill"></i>';
      button2.className =
        "btn btn-xs btn-outline-success px-1 py-0 rounded-circle";
      ...
      button3.innerHTML = '<i class="bi-pencil-square"></i>';
      button3.className =
        "btn btn-xs btn-outline-warning px-1 py-0 rounded-circle";
      ...
      button4.innerHTML = '<i class="bi-trash"></i>';
      button4.className =
        "btn btn-xs btn-outline-danger px-1 py-0 rounded-circle";
      button4.addEventListener("click", () => {
        // Handle button 3 click event
        this.handleButtonClick(rowData, 4);
      });
    ...
    handleButtonClick(rowData, buttonIndex) {
      ...
      this.sendData = this.posts.transData.find(
        (item) => item.header[0] === rowData
      );
    ...
    getContentWidth() {
      ...
      let columns = 0;
      const tes = this.hotSettings.columns.filter(
        (x) => x.type === "text" && !x.width
      ).length;
